    ...
    UIDChangerService(chip)
{
    ...
    {
        reader_unit_ = std::dynamic_pointer_cast<NFCReaderUnit>(
            getChip()->getCommands()->getReaderCardAdapter()
            ->getDataTransport()->getReaderUnit());
    }

    EXCEPTION_ASSERT_WITH_LOG(reader_unit_, LibLogicalAccessException, "Cannot retrieve an "
        "instance of NFCReaderUnit from the card.");
}

void MifareClassicUIDChangerService::changeUID(const std::vector<uint8_t> &new_uid)
{
    EXCEPTION_ASSERT_WITH_LOG(new_uid.size() == 4, LibLogicalAccessException,
